...
public class WebServerApp implements Runnable {
    ...
    public static void onStartUp() {
        ...
        for (int i = 0; i < db.getLoadedPortList().size(); i++) {
            ...
            try {
                webpage = new StaticWebPage(db.getLoadedHTMLList().get(i), db.getLoadedCSSList().get(i),
                        db.getLoadedJavaScriptList().get(i), db.getLoadedPortList().get(i));
    ...
    public void run() {
        ...
        try {
            in = new BufferedReader(new InputStreamReader(socket.getInputStream()));// read in all the inputs on the
                                                                                    // socket and build a buffer
            ...
            /*
             * the first part of the response is the header. the first line of the header
             * contains info such
             * HTTP method (GET,POST,PUT...etc) and resource and resource root (/people of
             * /people/jobs ...etc)
             */
            System.out.println();
            ...
            // more*/
                ...
                if (request.getResource().equals("/upload"))// check the resource we are looking for
                {
                    ...
                    response.sendResponse("200 OK", portList);
                    // db.setFiles(
                    // webpage.getPort(),webpage.getHTML(),webpage.getCSS(),webpage.getJavaScript());
