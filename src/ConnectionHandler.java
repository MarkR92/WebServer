        ...
        try {
            ...
            // more*/
                ...
                if (request.getResource().equals("/upload"))// check the resource we are looking for
                {
                    request.getBody();
                    // response.sendResponseOk();
                    //we need to read the file in rootnames.
                    //we read files using the stringbuilder stuff so all we need to do is copy the stringbuilder stuff to (here?)
                    StaticWebPage webpage = new StaticWebPage(response.findResource3());
