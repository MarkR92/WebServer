...
public class StaticWebPage implements Runnable {
    ...
    // the boday will contain the unedited uploaded files. We will parse them in
    // this class
    ...
    public void createStaticWebPage() throws IOException {
        ...
        while (true) {
            ...
            try (Socket client = webpageSocket.accept()) {
                ...
                in = new BufferedReader(new InputStreamReader(client.getInputStream()));// read in all the inputs on the
                ...
                if (request.getMethod().equals("GET")) {
                    ...
                    } else if (request.getResource().matches("^.*\\.css$"))// the root doesnt do anything rn. Just a
                                                                           // "welcome page"
                    {
                        client.getOutputStream().write(("HTTP/1.1 200 OK\r\n").getBytes());
                        client.getOutputStream().write(("Access-Control-Allow-Origin: *\r\n").getBytes());
                    ...
                    } else if (request.getResource().matches("^.*\\.js$"))// the root doesnt do anything rn. Just a
                                                                          // "welcome page"
                    {
                        client.getOutputStream().write(("HTTP/1.1 200 OK\r\n").getBytes());
                        client.getOutputStream().write(("Access-Control-Allow-Origin: *\r\n").getBytes());
